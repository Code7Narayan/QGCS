...
ApplicationWindow {
    ...
    QtObject {
        ...
        function clearNextPromptSignal() {
            if (currentDialog) currentDialog.closed.disconnect(nextPrompt)
        }
        function nextPrompt() {
    ...
    function pushPreventViewSwitch() { _rgPreventViewSwitch.push(true) }
    ...
    function preventViewSwitch() { return _rgPreventViewSwitch[_rgPreventViewSwitch.length - 1] }
    function viewSwitch(toolbar) {
    ...
    function showFlyView() {
        if (!flightView.visible) showPreFlightChecklistIfNeeded()
        viewSwitch(toolbar)
    ...
    function showAnalyzeTool() { showTool(qsTr("Analyze Tools"), "AnalyzeView.qml", "/qmlimages/Analyze.svg") }
    function showSetupTool() { showTool(qsTr("Vehicle Setup"), "SetupView.qml", "/qmlimages/Gears.svg") }
    function showSettingsTool() { showTool(qsTr("Application Settings"), "AppSettings.qml", "/res/QGCLogoWhite") }
    ...
    Popup {
        ...
        onOpened: criticalVehicleMessageText.text = mainWindow._vehicleMessage
        onClosed: {
        ...
        QGCColoredImage {
            ...
            MouseArea {
                anchors.fill: parent
                onClicked: criticalVehicleMessageFlick.flick(0,-500)
            }
        }
    }
    ...
    Popup {
        ...
        onOpened: loader.sourceComponent = indicatorPopup.currentIndicator
        onClosed: {

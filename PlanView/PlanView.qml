...
Item {
    ...
    function updateAirspace(reset) {
        if(_airspaceEnabled) {
            var coordinateNW = editorMap.toCoordinate(Qt.point(0,0), false)
            var coordinateSE = editorMap.toCoordinate(Qt.point(width,height), false)
            if(coordinateNW.isValid && coordinateSE.isValid) {
                QGroundControl.airspaceManager.setROI(coordinateNW, coordinateSE, true, reset)
            }
        }
    }
    ...
    PlanMasterController {
        ...
        function upload() {
            if (!checkReadyForSaveUpload(false)) return
    ...
    Connections {
        ...
        function onNewItemsFromVehicle() {
            if (_visualItems && _visualItems.count !== 1) mapFitFunctions.fitMapViewportToMissionItems()
            _missionController.setCurrentPlanViewSeqNum(0, true)
        }
    }
    ...
    QGCFileDialog {
        ...
        onAcceptedForSave: {
            if (planFiles) _planMasterController.saveToFile(file)
            else _planMasterController.saveToKml(file)
            close()
        }
        onAcceptedForLoad: {
    ...
    Item {
        ...
        FlightMap {
            ...
            onZoomLevelChanged: { QGroundControl.flightMapZoom = zoomLevel; updateAirspace(false) }
            onCenterChanged: { QGroundControl.flightMapPosition = center; updateAirspace(false) }
            MouseArea {
                anchors.fill: parent
                onClicked: {
                    ...
                    case _layerRallyPoints:
                        if (_rallyPointController.supported && addWaypointRallyPointAction.checked) _rallyPointController.addPoint(coordinate)
                        break
                    }
                }
            }
        ...
        ToolStrip {
            ...
            Rectangle {
                ...
                radius: _radius
                gradient: Gradient {
                    GradientStop { position: 0.0; color: qgcPal.globalTheme === QGCPalette.Light ? "#f5f7fa" : "#2c3e50" }
                    GradientStop { position: 1.0; color: qgcPal.globalTheme === QGCPalette.Light ? "#e0e6ed" : "#1a252f" }
                }
                DropShadow {
                    anchors.fill: parent
                    source: parent
                    ...
                    radius: 8.0
                    samples: 17
                    color: qgcPal.globalTheme === QGCPalette.Light ? "#20000000" : "#80ffffff"
                    cached: true
                }
            }
            ToolStripActionList {
                ...
                model: [
                    ...
                    ToolStripAction {
                        ...
                        onTriggered: {
                            toolStrip.allAddClickBoolsOff()
                            if (_singleComplexItem) insertComplexItemAfterCurrent(_missionController.complexMissionItemNames[0])
                        }
                    },
                    ToolStripAction {
        ...
        Item {
            ...
            Column {
                ...
                QGCTabBar {
                    ...
                    QGCTabButton { text: qsTr("Fence"); enabled: _geoFenceController.supported }
                    QGCTabButton { text: qsTr("Rally"); enabled: _rallyPointController.supported }
                }
            }
            Item {
        ...
        ColumnLayout {
            ...
            GridLayout {
                ...
                QGCButton {
                    ...
                    onClicked: {
                        dropPanel.hide()
                        if(_planMasterController.currentPlanFile !== "") _planMasterController.saveToCurrent()
                        else _planMasterController.saveToSelectedFile()
                    }
                }
                QGCButton {
                    text: qsTr("Save As...")

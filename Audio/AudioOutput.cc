...
//   are in English and not localized.
    ...
    // Select a better voice if available (female or natural voice)
    const auto voices = _tts->availableVoices();
    for (const QVoice& voice : voices) {
        if (voice.gender() == QVoice::Female) {
            _tts->setVoice(voice);
            qDebug() << "Using female voice:" << voice.name();
            _tts->say("Welcome aboard. This is Aerobotz Private Limited — your intelligent flight system is now online.");
